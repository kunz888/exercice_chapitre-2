...
const input = {
    language: 'Solidity',
    sources: {
        'Envoyer des ethers.sol': {
            content: fs.readFileSync("./exercices/ex06 - Envoyer des ethers/sources/Envoyer des ethers.sol", "utf-8")
        }
    },
    settings: {
        outputSelection: {
...
const abi = contracts['Envoyer des ethers.sol']['Payable'].abi;
const bytecode = contracts['Envoyer des ethers.sol']['Payable'].evm.bytecode;
...
mocha.describe('Payable', () => {
    ...
    mocha.it('We have sent money', async () => {
        const accounts = await getAccounts();
        const contract = await new web3.eth.Contract(abi)
            .deploy({ data: bytecode.object })
            .send({ from: accounts[0], gas: 2000000,value:web3.utils.toWei("2","ether")});
