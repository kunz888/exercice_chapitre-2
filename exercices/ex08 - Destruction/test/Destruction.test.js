...
const input = {
    ...
    sources: {
        'Destruction.sol': {
            content: fs.readFileSync("./exercices/ex08 - Destruction/sources/Destruction.sol", "utf-8")
        }
    },
    settings: {
        outputSelection: {
...
const abi = contracts['Destruction.sol']['Payable'].abi;
const bytecode = contracts['Destruction.sol']['Payable'].evm.bytecode;

const getAccounts = async () => {
    return await web3.eth.getAccounts();
};

mocha.describe('Destruction', () => {
    ...
    mocha.it('We have destroy and send money', async () => {
       ...
       const tx = await contract.methods.destroySmartContract().send({from:accounts[0]});
       console.log(tx);
